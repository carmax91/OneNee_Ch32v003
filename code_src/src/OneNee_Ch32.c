...
//
// PAL PM-41 consoles PAL BIOS supported ONLY!

// PINOUT for wch ch32v003j4m6:
/*
                   ___ ___
  Bios_D2 (PD6) - |   U   |- Bios_A18 (PD5) (SWIO)
             GND -|       |- SUBQ (PC4)
...
#define delay_BFix 2600 //Optimized delay for CH32V003 MCU @48MHz
...
void NTSC_fix() {
  Delay_Ms(delay_BFix); //Optimized delay for CH32V003 MCU @48MHz

  __disable_irq(); // start critical section
  
  while (!(GPIOD->INDR >> (5 & 0xf) & 1)) //(read (bios_A18) == 0) 
  {
    ;  //wait for priming A18 pulse
  }
  
  Delay_Us(12); //min 13us max 17us for 16Mhz ATmega (maximize this when tuning!)
  
  // PD6 -> Bios_D2 Output ------------------------------------------------
  GPIOD->CFGLR &= ~(0xF << (4 * 6));
  GPIOD->CFGLR |= (3 | 0) << (4 * 6); // D2 = output. Drags line low now
  //------------------------------------------------------------------
  GPIOD->BSHR = (1 << 16 + 6); // PC6 -> Bios_D2 Low // store a low
  
  
  Delay_Us(5); //min 2us for 16Mhz ATmega, 8Mhz requires 3us (minimize this when tuning, after maximizing first us delay!)
  
  // PD6 <- Bios_D2 Input ------------------------------------------------
  GPIOD->CFGLR = (GPIOD->CFGLR & (~(0xf<<(4*(6&0xf))))) | (4<<(4*(6&0xf)));
  //-----------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------
  // PD5 <- Bios_A18 Input ------------------------------------------------
  GPIOD->CFGLR = (GPIOD->CFGLR & (~(0xf<<(4*(5&0xf))))) | (4<<(4*(5&0xf)));
  //-----------------------------------------------------------------------
}
